...
async def _run(question: str, db_path: Path, verbose: bool) -> int:
...
def main() -> None:
    ...
    if not args or args[0] in ("-h", "--help"):
        print("Usage: data-assistant [--db PATH] [--verbose] <question>")
        print()
        print("Examples:")
        print('  data-assistant "What was our top product last month?"')
        print('  data-assistant --verbose "How many customers signed up in the last 30 days?"')
        print('  data-assistant --db /path/to/db.sqlite "What is our total revenue?"')
        sys.exit(0)
    ...
    sys.exit(asyncio.run(_run(question, db_path, verbose)))
